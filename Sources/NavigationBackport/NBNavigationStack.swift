...
public struct NBNavigationStack<Root: View, Data: Hashable>: View {
    ...
    @State var path: NavigationPathHolder
    @State var destinationBuilder = DestinationBuilderHolder()
    ...
    public var body: some View {
        WithPerceptionTracking {
            content
                .environment(path)
                .environment(Unobserved(object: path))
                .environment(destinationBuilder)
                .environment(Navigator(useInternalTypedPath ? $internalTypedPath : $externalTypedPath))
                .onFirstAppear {
    ...
    public init(path: Binding<[Data]>?, @ViewBuilder root: () -> Root) {
        ...
        self.root = root()
        _path = State(wrappedValue: NavigationPathHolder(path: path?.wrappedValue ?? []))
